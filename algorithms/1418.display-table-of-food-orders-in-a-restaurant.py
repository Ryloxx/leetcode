...
# Testcase Example:  '[["David","3","Ceviche"],["Corina","10","Beef
# Burrito"],["David","3","Fried Chicken"],["Carla","5","Water"],
...
# Waffles"],["Brianna","1","Canadian Waffles"]] Output: [["Table","Canadian
...
# Burrito"],["Melissa","2","Soda"]] Output: [["Table","Bean
# Burrito","Beef Burrito","Soda"],["2","1","1","1"]]
#
#
#
# Constraints:
...
class Solution:
    ...
    def displayTable(self, orders: List[List[str]]) -> List[List[str]]:
        dishes, memo = set(), defaultdict(lambda: defaultdict(int))
        for _, table, dish in orders:
            memo[table][dish] += 1
            dishes.add(dish)
        dishes = sorted(dishes)
        res = [["Table"] + dishes]
        for x in sorted(memo.keys(), key=lambda a: int(a)):
            row = [x]
            for y in dishes:
                row.append(str(memo[x][y]))
            res.append(row)
        return res
...
if __name__ == "__main__":
    run(
        MethodType(Solution.displayTable, Solution()),
