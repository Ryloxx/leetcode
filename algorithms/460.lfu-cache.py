...
class LFUCache:
    ...
    def __init__(self, capacity: int):
        self.d = dict()
        self.freq = defaultdict(dict)
        self.cap = capacity
        self.m = float("inf")

    def updateminfreq(self):
        if self.freq:
            self.m = min(self.freq.keys())

    def delfromfreq(self, key, freq):
        deleted = False
        if freq in self.freq and key in self.freq[freq]:
    ...
    def updatefreq(self, key, freq):
        self.freq[freq][key] = None
        self.delfromfreq(key, freq - 1)

    def removelfu(self):
        if not self.d:
            return False
        keytoremove = next(iter(self.freq[self.m]), None)
        if keytoremove is None:
            return False
        if self.delfromfreq(keytoremove, self.m):
            del self.d[keytoremove]
            return True
        return False

    def update(self, key, newV=None):
        val = None
        if key in self.d:
            val = self.d[key] if newV is None else [newV, self.d[key][1]]
            del self.d[key]
            val[1] += 1
        else:
            if newV is None:
            ...
            val = [newV, 1]
        self.d[key] = val
        self.updatefreq(key, val[1])
        return val

    def get(self, key: int) -> int:
        if key in self.d and (ret := self.update(key)):
            return ret[0]
        return -1
...
# param_1 = obj.get(key)
# obj.put(key,value)# @lc code=end
if __name__ == "__main__":
    run(
        wrapp_class(LFUCache),
        [
            ([[2],
