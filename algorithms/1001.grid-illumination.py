...
class Solution:
    ...
    # N = grid size, L = lamp count, Q = query count
    # O(L + Q) time complexity
    ...
    def gridIllumination(self, n: int, lamps: List[List[int]],
        ...
        for (y, x) in lamps:
            for p in enumerate([y, x, y + x, y - x]):
                buckets[p] += 1

        def is_in_reach(y, x):
            return int(
                any(buckets[p] > 0 for p in enumerate([y, x, y + x, y - x])))
...
if __name__ == "__main__":
    run(
        MethodType(Solution.gridIllumination, Solution()),
        [
