    ...
    def pseudoPalindromicPaths(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node: TreeNode):
            ...
            if not node:
                return 0
            res = 0
            odd_count ^= 1 << node.val
            if not node.right and not node.left:
                res = int(not odd_count & odd_count - 1)
            else:
...
if __name__ == "__main__":
    run(
        MethodType(Solution.pseudoPalindromicPaths, Solution()),
        [
            [[TreeNode.create_tree([2, 3, 1, 3, 1, None, 1])], 2],
            [[
                TreeNode.create_tree(
                    [2, 1, 1, 1, 3, None, None, None, None, None, 1])
            ], 1],
            [[TreeNode.create_tree([9])], 1],
        ],
    )
