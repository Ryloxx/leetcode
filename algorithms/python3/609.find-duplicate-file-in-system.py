...
# Testcase Example:  '["root/a 1.txt(abcd) 2.txt(efgh)","root/c 3.txt(abcd)"
...
class Solution:
    ...
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        ...
        for info in paths:
            dir_path, *files = info.split()
            for file in files:
                file_name, content = file.split('(')
                dup[content].append("%s/%s" % (dir_path, file_name))
        return list(filter(lambda x: len(x) > 1, dup.values()))


# @lc code=end
if __name__ == "__main__":
    run(MethodType(Solution.findDuplicate, Solution()), [
        [[[
        ...
        comparator=lambda a, b: sorted(map(sorted, a)) == sorted(map(
            sorted, b)))
