...
class Solution:
    ...
    def addOneRow(self, root: Optional[TreeNode], val: int,
        ...
        if depth == 2:
            return self.addOneRow(root, val, -1)
        if depth < 0:
            root.left, root.right = TreeNode(val, root.left), TreeNode(
                val, None, root.right)
        else:
...
if __name__ == "__main__":
    run(MethodType(Solution.addOneRow, Solution()), [
        [[TreeNode.create_tree([4, 2, 6, 3, 1, 5]), 1, 2],
         TreeNode.create_tree([4, 1, 1, 2, None, None, 6, 3, 1, 5])],
        [[TreeNode.create_tree([4, 2, None, 3, 1]), 1, 3],
         TreeNode.create_tree([4, 2, None, 1, 1, 3, None, None, 1])],
    ],
